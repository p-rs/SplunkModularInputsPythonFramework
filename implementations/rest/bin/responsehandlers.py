...
'''
    ...
    # passes the raw output directly to STDOUT
'''
...
class DefaultResponseHandler:
    ...
    def __call__(self, response_object,
                 raw_response_output, response_type,
                 req_args, endpoint):
...
'''
    Template Response Handlers
'''


'''
class CloudflareFirewallEventsHandler:
    ...
    def __call__(self, response_object,
        ...
        if response_type == "json":
            ...
            while offset is not None:
                ...
                for record in output["records"]:
                    print_xml_stream(json.dumps(record))
                # hopefully (guessing) at the end of the pagination
                # no more "offset" values in the JSON response
                # so this will cause the while loop to exit
                if "offset" in output:
                    offset = output["offset"]
                else:
                    offset = None
    ...
    def __call__(self, response_object,
        ...
        if response_type == "xml":
            ...
            for alarm in alarm_list:
                alarm_xml_str = ET.tostring(
                    alarm, encoding='utf8', method='xml')
...
class AutomaticEventHandler:
    ...
    def process_automatic_response(data):
        ...
        for event in output:
            # each element of the array is written to Splunk as seperate events
    ...
    def __call__(self, response_object,
        ...
        if response_type == "json":
            ...
            if "next_stream_position" in output:
                req_args["params"]["stream_position"] = (
                output["next_stream_position"])
...
class HPEResponseHandler:
    ...
    def __call__(self, response_object,
        ...
        if response_type == "json":
            output = json.loads(raw_response_output)
            new_watermark = None
            # split out each event and keep track of
            # latest update time for new watermark value
            for event in output["event_list"]["event"]:
                time_changed = datetime.strptime(
                    event["time_changed"][:23], self.date_format)
            ...
            req_args["params"]["watermark"] = datetime.strftime(
                new_watermark, self.date_format)
...
class SmartTabHandler:
    ...
    def __call__(self, response_object,
        ...
        if response_type == "json":
            ...
            # increment the date parameters by 1 day
            # These will get automagically persisted
            # back to inputs.conf for you
            req_args["params"]["dateStart"] = increment_one_day(
                req_args["params"]["dateStart"])
            req_args["params"]["dateEnd"] = increment_one_day(
                req_args["params"]["dateEnd"])
...
def print_xml_stream(s):
    print ("<stream><event unbroken=\"1\"><data>%s</data>"
           "<done/></event></stream>" % encodeXMLText(s))
